...
public class Base64Codec {
	...
	/**
	 * base64编码
	 * @param msg 用于编码为base64的字节数组
	 * @return 编码(字符串)
	 */
	public static String base64Encode(byte[] msg){
		BASE64Encoder base64encoder = new BASE64Encoder();
		return base64encoder.encode(msg);
	}

	/**
	 * base64解码
	 * @param msg 用于进行base64解码的字符串
	 * @return 译码(字节数组)
	 */
	public static byte[] base64Decode(String msg) {
		BASE64Decoder base64decoder = new BASE64Decoder();
		try {
			return base64decoder.decodeBuffer(msg);
		} catch (IOException e) {
			e.printStackTrace();
			return null;
		}
	...
	public static void main(String[] args) {
		...
		try {
			...
			while (true) {
				...
				String enCode = Base64Codec.encode(in);

				String deCode = Base64Codec.decode(enCode);
				System.out.println();
